...
function geofenceSummary(locations = []) {
  ...
  if (geofence.geofence_status === "OUTSIDE") return `<small class="geofence-OUTSIDE">Fora da cerca: ${Math.round(geofence.geofence_distance_meters || 0)} m de ${h(geofence.geofence_reference || "local")}${geofence.geofence_reason ? ` · ${h(geofence.geofence_reason)}` : ""}</small>`;
  if (geofence.geofence_status === "INACTIVE_FOR_PUNCH") return `<small class="geofence-INACTIVE_FOR_PUNCH">Cerca flexível nesta marcação</small>`;
  if (geofence.geofence_status === "DISABLED") return `<small class="geofence-DISABLED">Cerca desativada</small>`;
  if (geofence.geofence_status === "NOT_CONFIGURED") return `<small class="geofence-NOT_CONFIGURED">Cerca não configurada</small>`;
